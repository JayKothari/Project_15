...
#define NO_TASK		4	//number of task descripter
//#define TASK_PRIO	0
...
void rt_task_1(void *cookie)
{
	...
	{	
	       ...
	       // if(ret<0) { xnprintf("error in rt_task_inquire()  %d ...\n",ret); return ;}
             ...
             rt_task_wait_period(&overrun_t);				//wait
	     xnprintf("overrun1=%lu\n",overrun_t);
...
{	
	...
	for(i=1; i<=NO_ITERATION; ++i){	
	       ...
	       // if(ret<0) { xnprintf("error in rt_task_inquire()  %d ...\n",ret); return ;}
             ...
             rt_task_wait_period(&overrun1_t);				//wait
	     xnprintf("overrun2=%lu\n",overrun1_t);
             cur_sys_time = rt_timer_read();			//read the sys_time
             xnprintf("iteration %d ..actual task wake-up....value %llu \n", i, cur_sys_time);
             jitter = cur_sys_time - expected_time;
             xnprintf("iteration %d ..actual task wake-up....value %llu \n", i, cur_sys_time);
...
void rt_task_3(void *cookie)
{
	...
	for(i=1; i<=NO_ITERATION; ++i){	
	       ...
	       // if(ret<0) { xnprintf("error in rt_task_inquire()  %d ...\n",ret); return ;}
             ...
             rt_task_wait_period(&overrun3_t);				//wait
             xnprintf("overrun3=%lu\n",overrun3_t);
...
void rt_task_4(void *cookie)
{
	...
	for(i=1; i<=NO_ITERATION; ++i){	
	       ...
	       // if(ret<0) { xnprintf("error in rt_task_inquire()  %d ...\n",ret); return ;}
             ...
             rt_task_wait_period(&overrun4_t);
	     xnprintf("overrun4%lu\n",overrun4_t);				//wait
...
END:ret = 0;
	...
	int rt_task_prio[NO_TASK] = {1,99,2,3};	//set the priority for each task
